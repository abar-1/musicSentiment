...
function HomePage() {
  ...
  const handleGenerate = async () => {
    if (selectedGenres.length === 0) {
      alert("Please select genre(s)");
      return;
    }
    if (!analysisDone || detectedEmotion === null || detectedEmotion === "No emotions detected") {
      alert("Please run emotion analysis");
      return;
    }
    ...
    try {
      const requestData = {
        user_mood: detectedEmotion,
        genres: selectedGenres
      };

      const res = await fetch('http://localhost:5000/get_songs', {
        method: 'POST',
        headers: {
          "Content-Type": "application/json"
        },
